    ...
    try:
        while True:
            ...
            if data.get("type") == "init":
                ...
                # Route first so we send the correct agent_start event
                orchestrator = services["orchestrator"]
                context = orchestrator.get_or_create_context(session_id, student_id)
                if target_agent:
                    from api.services.orchestrator import AgentSelection
                    selection = AgentSelection(agent=target_agent, protocol="DIRECT_TUTORING")
                else:
                    selection = orchestrator.route(message, context)

                routed_agent_id = selection.agent
                agent = services["agents"].get_agent(routed_agent_id)
                await websocket.send_json({
                    "type": "agent_start",
                    "agent_id": routed_agent_id,
                    "agent_name": agent.name if agent else routed_agent_id,
                    "color": agent.color if agent else "#58a6ff",
                    "icon": agent.icon if agent else "🤖",
                })

                # Process message (pass pre-computed target so it isn't re-routed)
                result = await process_chat(student_id, session_id, message, routed_agent_id)
...
async def update_student(student_id: str, data: StudentUpdate):
    ...
    services["db"].update_student(student_id, **updates)
    return services["db"].get_student(student_id)
